...
export default function CartView() {
  const { items, removeItem, updateQuantity, getTotalPrice, clearCart, getItemCount } = useCart();
  ...
  if (isLoading) { // Show skeleton for the whole cart structure including summary
    return (
  ...
  if (getItemCount() === 0) {
    return (
      <div className="text-center py-12 bg-card p-8 rounded-lg shadow-lg">
        ...
        <p className="text-muted-foreground mb-8">Looks like you haven't added any artwork yet. Explore our collection!</p>
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-6">
        ...
        {items.map((item) => (
          <Card key={item.product.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 shadow-sm">
            ...
            <div className="flex items-center space-x-2 sm:ml-auto shrink-0">
              ...
              <Button variant="ghost" size="icon" onClick={() => removeItem(item.product.id)} className="text-destructive hover:bg-destructive/10 hover:text-destructive h-8 w-8">
