...
export default function ArtworkUploadForm({
  ...
  const onFormSubmit: SubmitHandler<ArtworkFormValues> = async (data) => {
    ...
    try {
      ...
      if (response.ok && result.status === 1) {
        toast({
          title: "Artwork Saved!",
          description: "Your artwork has been successfully submitted.",
          variant: "success",
        });
        onSubmitSuccess();
      } else {
  ...
  const buttonText = isStandalone ? "Submit Artwork" : "Save & Continue";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                {multiShotPreviews.map((preview, index) => (
                  <div key={index}>
                    <input
                      ...
                      className="hidden"
                      accept="image/*"
                      disabled={isUploadingMulti[index]}
                    />
                    <div
          ...
          <div className="lg:col-span-3 space-y-4">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    onValueChange={field.onChange}
                    value={field.value || ""}
                    disabled={isFetchingMaterials || materials.length === 0}
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue
            ...
            <p className="text-xs text-muted-foreground">
              Works should be shipped unframed and in in roll unless rolling is
              not possible. Please write to partners@mojarto.com to change
              delivery condition
            </p>
            ...
            <div className="space-y-4">
                    ...
                    <FormLabel>
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>
                              The final price after discount. This is calculated
                              automatically.
                            </p>
                          </TooltipContent>
                        </Tooltip>
              ...
              <div className="grid md:grid-cols-2 gap-x-4 gap-y-2 items-center">
                ...
                <p className="text-xs text-muted-foreground self-end pb-2">
                  Please ensure you have accounted for shipping charges in your
                  price quotation. Please revise the same, if you need to,
                  before submitting this artwork.
                </p>
              </div>
