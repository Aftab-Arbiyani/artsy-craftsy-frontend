...
export default function ProductDetails({
  ...
  const [product, setProduct] = useState<Product | undefined | null>(
    initialProduct !== undefined ? initialProduct : undefined
  );
  const [isLoading, setIsLoading] = useState(initialProduct === undefined);
  const [selectedImage, setSelectedImage] = useState<string | null>(
    initialProduct?.imageUrls?.[0] ?? null
  );
  ...
  useEffect(() => {
    ...
    // If initialProduct was passed from the server, skip the main fetch
    if (initialProduct !== undefined) {
      if (initialProduct) fetchRelated(initialProduct);
      return;
    }
    ...
    const fetchProductAndRelated = async () => {
      ...
      try {
        ...
        if (productResult.status === 1 && productResult.data) {
          ...
          await fetchRelated(transformedProduct);
        } else {
          setProduct(null);
        }
      } catch (error) {
